...
%\VignetteIndexEntry{Package 'iapws'}
...
(\code{h}) are computed simultaneously for several temperatures
(\code{t}) between 573 K and 623 K.
...
If more interested in isobaric properties, one can use the function
\fun{iapws95\_pt} instead:
